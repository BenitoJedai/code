...
namespace RayCaster1.source.java
{
    ...
    partial class RayCaster1Base
    {
        ...
        public Color GetWallColor(sbyte w, bool alt)
        {
            if (alt)
            {
                if (w == R) return Color.FromArgb(0xaf0000);
                if (w == G) return Color.FromArgb(0x00af00);
                if (w == B) return Color.FromArgb(0x0000af);
            }

            if (w == R) return Color.FromArgb(0xbf0000);
            if (w == G) return Color.FromArgb(0x00bf00);
            if (w == B) return Color.FromArgb(0x0000bf);
