...
namespace RayCaster1.source.java
{
    ...
    public partial class RayCaster1Base  // Applet, Runnable
    {
        ...
        public Action<int, int, int, int> fillRect;
        public Action<int, int, int, int> drawLine;
        ...
        public void drawBackground()
        {
            ...
            for (r = 0; r < PROJECTIONPLANEHEIGHT / 2; r += 10)
            {
                ...
                setColor(Color.FromArgb(red, 125, 225));
                fillRect(0, r, PROJECTIONPLANEWIDTH, 10);
                c += 20;
            }
            // ground
            c = 22;
            for (; r < PROJECTIONPLANEHEIGHT; r += 15)
            {
                ...
                setColor(Color.FromArgb(20, red2, 20));
                fillRect(0, r, PROJECTIONPLANEWIDTH, 15);
                c += 15;
            }
        }
        ...
        public void drawOverheadMap()
        {
            ...
            for (int u = 0; u < MAP_WIDTH; u++)
            {
                for (int v = 0; v < MAP_HEIGHT; v++)
                {
                    if (myMap[u, v] != O)
                    {
                        setColor(Color.FromArgb(0x00ff00));
                    }
                    else
                    {
                        setColor(Color.FromArgb(0x002000));
                    }
                    fillRect(PROJECTIONPLANEWIDTH + (u * fMinimapWidth),
        ...
        {
            setColor(Color.FromArgb(0xffff00));
            // draw line from the player position to the position where the ray
            // intersect with wall
            drawLine(fPlayerMapX, fPlayerMapY,
            ...
            setColor(Color.FromArgb(0xff0000));
            drawLine(fPlayerMapX, fPlayerMapY,
