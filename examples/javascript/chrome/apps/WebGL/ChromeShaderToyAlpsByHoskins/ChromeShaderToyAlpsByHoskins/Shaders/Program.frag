...
vec3 GetSky(in vec3 rd)
{
	...
	vec3  sky = vec3(v*sunColour.x*0.4+.01, v*sunColour.y*0.4+0.06, v*sunColour.z*0.3+.15);
	// Wide glare effect...
	sky = sky + sunColour * pow(sunAmount, 8.0)*.9;
	// Actual sun...
	sky = sky+ sunColour * min(pow(sunAmount, 800.0), .3)*.8;
	return sky;
}
...
void DoLighting(inout vec3 mat, in vec3 pos, in vec3 normal, in vec3 eyeDir, in float dis)
{
	...
	mat = mat * sunColour * c * vec3(.9, .9, 1.0) +  ambient * vec3(.8, .9, 1.0);
	// Specular...
	if (h > 0.0)
	{
		vec3 R = reflect(sunLight, normal);
		float specAmount = pow( max(dot(R, normalize(eyeDir)), 0.0), 55.0)*specular * 2.0;
		mat += sunColour*vec3(1.0, .8, .5) * specAmount;
	}
}
...
vec3 TerrainColour(vec3 pos, vec3 normal, float dis)
{
	...
	float f = clamp(Noise(pos.xz*.01), 0.0,1.0);//*10.8;
	f *= Noise(pos.zx*.2+normal.xz*3.5);
	//f *= .5;
	mat = mix(vec3(0.0), vec3(.12, .07, .05), f);

	// Snow...
	if (pos.y > 75.0 && normal.y > .5)
	{
		float snow = smoothstep(0.0, 1.0, (pos.y - 75.0 - Noise(pos.xz * .03)*50.0) * 0.2 * (normal.y-.5));
...
vec3 CameraPath( float t )
{
	...
	return vec3(0.0+p.x, 0.6, -200.0+p.y);
}
...
void mainImage( out vec4 fragColor, in vec2 fragCoord )
{
    ...
    bri = ((cloud.y)) * bri;
    col = mix(col, vec3(min(bri+cloud.x * vec3(1.0,.9, .9), 1.0)), min(cloud.y*(bri+1.0), 1.0));
