...
vec3 flx(vec2 p,float ct){
     float t; vec3 q=vec3(p,p.x-p.y+iGlobalTime*0.3);
...
void main_(void){
     float sc,de,st,t=mod(iGlobalTime,55.);
     sc=5.;if(t<=sc){
        de=2.;st=scv(t/(sc-de));
        vr8=mix(.024,1. ,st);
        vr10=mix(2.37,1.,st);
...
void mainImage( out vec4 fragColor, in vec2 fragCoord ){
     float a=iGlobalTime*94.24777960;
     vec2 pa=.15*vec2(sin(a),cos(a));
     float asp=iResolution.x/iResolution.y;
     vec2 p=(pa+fragCoord.xy)/iResolution.xy-0.5; main_();
     p*=1./(1.-(vr9-1.)*length(p));
     p*=1.-length(p)*2.*(vr10-1.); p*=vr8; vec2 pp=p*2.; p.y*=3./asp; p*=1.2; float t=iGlobalTime*0.4;
     ...
     p.x*=3.; p=(0.5*p)+nd.xy*0.6*e; t=iGlobalTime*0.04;
     ...
     float d,ii=1.;e=0.02;
     for(float i=0.;i<64.;i++){d=length(r*vec3(0.1,.5,1.))-1.;q=r; q.y-=2.; d+=(pn(q+vec3(.0,iGlobalTime*2.,.0))+pn(q*3.)*.5)*.25*(q.y); d=min(100.-length(q),abs(lf+d))+e; r+=d*dir; if(d<e){ii=i/64.;break;}}
