...
void rect(vec4 _p,vec3 _c, in vec2 fragCoord, out vec4 fragColor)
{
	vec2 p = fragCoord.xy;
...
void print(float _i,vec2 _f,vec2 _p,vec3 _c, in vec2 fragCoord, out vec4 fragColor)
{
...
void draw_bg(vec2 p, out vec4 fragColor)
{
    fragColor = vec4(mod(p.x + p.y, 2.) * .05 + BG, 1);
}

void draw_text(vec2 p, in vec2 fragCoord, out vec4 fragColor)
{
    print(val, vec2(1), vec2(iResolution.x - 50.,10.), vec3(0), fragCoord, fragColor);
}

void draw_01x32(vec2 p, vec2 uv, out vec4 fragColor)
{
...
void draw_04x08(vec2 p, vec2 uv, out vec4 fragColor)
{
...
void handle_mouse(vec2 p, out vec4 fragColor)
{
    if (iMouse.z > 0.) {
