...
const vec3 eyeOffMono = vec3(0.0);
const vec3 headPosition = vec3(0.0);
...
*/
    ...
    vec3 cameraPos = headPosition;

    //WHY ARE EYE OFFSETS REVERSED??? (are FOVs reversed too?? is this related to negative Z values?)
    cameraPos -= eyeOff.x*cameraRight;
    cameraPos -= eyeOff.y*cameraUp;
    cameraPos -= eyeOff.z*cameraFwd;
