...
#define MAGIC_ANGLE 0.868669484932732 // radians
...
/* Return a permutation matrix whose first two rows are u and v basis 
   vectors for a cube face, and whose third row indicates which axis 
   (x,y,z) is maximal. */
void getPT(in vec3 p, out mat3 PT, out float l) {
...
/* For any point in 3D, obtain the permutation matrix, as well as grid coordinates
   on a cube face. */
void posToGrid(in vec3 pos, out mat3 PT, out vec3 gl) {
    ...
    // Unwarp through arctan function - note tan(theta) should simplify to 1.0
...
vec3 gridToPos(in vec3 g, in mat3 PT) {
    
    // Map [0,N] to [-1,1]
    vec2 q = g.xy/N * 2.0 - 1.0;
    
    // Warp through tangent function - note tan(theta) should simplify to 1.0
    vec2 p = tan(warp_theta*q)/tan_warp_theta;

    // Map back through permutation matrix to place in 3D.
    return PT * vec3(p, 1.0)*sign(g.z);
...
*/
bool gridNeighbor(in mat3 PT, in vec3 g, in vec2 delta, out mat3 PTn, out vec3 gn) {

    vec2 uv_dst = g.xy + delta;
    vec2 uv_dst_clamp = clamp(uv_dst, 0.0, N);

    vec2 extra = abs(uv_dst_clamp - uv_dst);

    gn = vec3(uv_dst_clamp, g.z);
    PTn = PT;

    if (max(extra.x, extra.y) == 0.0) {
        return true;
    } else if (min(extra.x, extra.y) == 0.0) {
        // Magic stuff happens here.
        float retract = dot(extra, vec2(sign(g.z)*2.0/N));
        float q = 1.0 - retract;
        float p = tan(warp_theta*q)/tan_warp_theta;
        
        vec3 pos = gridToPos(gn, PT) - (1.0-p) * PT[2];
        posToGrid(pos, PTn, gn); 
        return true;	        
    } else {
        return false;
    }
    ...
    for (float u=-1.0; u<=1.0; ++u) {
        for (float v=-1.0; v<=1.0; ++v) {
            ...
            if (gridNeighbor(PT, g, vec2(u,v), PTn, gn)) {
                ...
                gn.xy = floor(gn.xy);
                vec3 rn = hash3(gn.xy*0.123 + 2.0*gn.z);
                gn.xy += 0.5 + (rn.xy * 2.0 - 1.0)*randomize_amount*0.5;

                // Get the 3D position
                vec3 pos_n = gridToPos(gn, PTn);
                
                // Compute squared distance on sphere
                float dp = sphereDist2(pos, pos_n);
                
                // Compute material
                float mp = mix(planeToColor(gn.z) + rn.z/12.0, rn.z, color_scheme);
