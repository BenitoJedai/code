...
{*/
    ...
    {
        return length(p)-4.5; // The coefficient is a bit ad-hoc here. The goal is to quickly catch a "leaking ray".
    }
	...
	vec4 renderMaterial(vec2 fragCoord, vec3 p, vec3 n, float m)
	{
        ...
        #ifndef NO_EFFECTS
        if(floor(mod(m,100.0))==0.0 && d<50.0)
