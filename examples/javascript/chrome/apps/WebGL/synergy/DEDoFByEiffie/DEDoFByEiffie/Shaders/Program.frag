...
float FuzzyShadow(vec3 ro, vec3 rd, float coneGrad, float rCoC, vec2 px){
	...
	for(int i=0;i<8;i++){
		...
		d=DE(ro+rd*t)+r*0.5;
		s*=linstep(-r,r,d);
		t+=abs(d)*(0.8+0.2*rand(px*vec2(i)));
	}
...
void mainImage( out vec4 fragColor, in vec2 fragCoord ) {
	...
	vec3 rd=lookat(-ro*vec3(1.0,2.0,1.0)-vec3(1.0,0.0,0.0),vec3(0.0,1.0,0.0))*normalize(vec3((2.0*fragCoord.xy-size.xy)/size.y,1.0));
	...
	for(int i=1;i<48;i++){//march loop
		...
		if(d<rCoC){//if we are inside add its contribution
			...
			//if(dot(N,rd)<=0.0){//it doesn't seem to matter if we take samples around the corners!
				...
				scol*=FuzzyShadow(p,L,shadowCone,rCoC,fragCoord);//I wouldn't have believed this would compile but only 8 shadows steps!!!
		...
		d=abs(d)*(0.8+0.2*rand(fragCoord.xy*vec2(i)));//add in noise to reduce banding and create fuzz
		ro+=d*rd;//march
		t+=d;
	...
	fragColor = vec4(clamp(2.0*col.rgb,0.0,1.0),1.0);
}
