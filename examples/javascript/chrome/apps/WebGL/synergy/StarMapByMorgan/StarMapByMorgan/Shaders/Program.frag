...
vec3 sun(vec3 d) {
	...
	vec3 core = vec3(2.8, 1.5 + 0.5 * noise(time * 0.25 + d.xy * 5.0), 1.5) * falloff; 
    float corona = NOISE(vec2(d.z * 250.0 + time, time * 0.2 + angle * 50.0)) * smoothstep(0.95, 0.92, d.z) * falloff * square(d.z);
...
vec4 planet(vec3 view) {
    ...
    vec3 dir = s;
    dir = rotation(0.0, time * 0.01) * dir;
    ...
    vec4 cloud = vec4(vec3(1.5),
                      ...
                      square(NOISE(vec2(time * 0.1, 0.0) + dir.xz * 11.0 * square(dir.y) + dir.yx * 3.0 + dir.zy * 1.2)));
    ...
    if (length(spherePoint) <= 1.0) {
        ...
        for (int x = -3; x <= 3; ++x) {
	        for (int y = -3; y <= 3; ++y) {
		        ...
		        dir = rotation(iGlobalTime, -iGlobalTime * 0.17) * vec3(s.xy, sqrt(max(0.0, 1.0 - dot(s.xy, s.xy))));
				c += sphereColor(dir);
