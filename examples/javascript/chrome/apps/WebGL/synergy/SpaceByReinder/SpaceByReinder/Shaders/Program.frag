...
const int   RING_VOXEL_STEPS = 30;
const vec3  RING_COLOR_1 = vec3(0.42,0.3,0.2) * 0.6;
const vec3  RING_COLOR_2 = vec3(0.51,0.41,0.32);

const int   ASTEROID_NUM_STEPS = 12;
...
#define SUN_DIRECTION normalize( vec3( 1.,  0., .2 ) )
...
float rock( const in vec3 p, const in vec3 id ) {  
    ...
    for(int i = 0; i < 9; i++) {
        ...
        float r = (ASTEROID_RADIUS*2.5) + ASTEROID_RADIUS*hash11(ii);
...
void asteroidTransForm(inout vec3 ro, const in vec3 id ) {
    float xyangle = (id.x-.5)*iGlobalTime;
    ro.xy = rotate( xyangle, ro.xy );
    
    float yzangle = (id.y-.5)*iGlobalTime;
...
vec3 asteroidGetStoneColor(vec3 p, float c, vec3 l, vec3 n, vec3 e) {
   vec3 color = RING_COLOR_1;
	...
	float fresnel = pow(1.0-abs(dot(n,e)),5.);
    color = mix( diffuse(n,l)*color, sunColour*specular(n,l,e,3.0),fresnel);    
...
vec4 renderStars( const in vec3 rd ) {
    ...
    s += noise(rds.xz*370.);
    s += noise(rds.xz*870.);
    s = pow(s,19.0) * 0.0000005;
    if (s > 0.5) {
        vec3 backStars = vec3(s)*.1; 
        col += backStars;
    }
	return   vec4( clamp(col,vec3(0),vec3(1)), 1 ); 
} 
...
vec4 renderPlanet( const in vec3 ro, const in vec3 rd, inout float maxd ) {
    ...
    maxd = d;
    vec3 col = vec3( .2, 7., 4. ) * .05;
    
    col *= diffuse( normalize( ro+rd*d ), SUN_DIRECTION );
...
//    camint( up, t, 6., vec3(    0. ,  -0.3,    1. ), pup, dup ); 
    ...
    camint( ro, t, 3., vec3(-1360. , 1800. , 1.2 ), pro, dro );
    camint( ta, t, 1., vec3(    0. , 200. ,-600. ), pta, dta );
    camint( up, t, 6., vec3(    0. ,  0.1,    1. ), pup, dup );

    camint( ro, t, 15., vec3(-1359.95 , 1800.11 , 1.19 ), pro, dro );
    camint( ta, t, 19., vec3(    0. , 20. ,   600. ), pta, dta );
    camint( up, t, 14., vec3(    0. ,  0.3,    1. ), pup, dup );
    
    camint( ro, t, 7., vec3(2900.5 , 4102. , 200.5 ), pro, dro );
...
void mainImage( out vec4 fragColor, in vec2 fragCoord ) {
    ...
    vec4 atmosphere = renderAtmospheric( ro, rd );
	...
	col += vec3(1.4,1.2,1.0)*lensflare(p, sunuv)		*clamp( flare, 0., 1.);
