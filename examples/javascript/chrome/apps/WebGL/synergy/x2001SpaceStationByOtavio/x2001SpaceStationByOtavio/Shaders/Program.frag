...
float localTime = 0.0;
float earthRad = 6371.0;
vec3 earthPos = normalize(vec3(-6500,-6400,2400)) * (earthRad + 300.0);
...
vec2 DistanceToObject(vec3 p0)
{
    float material = 0.0;
    float h = 0.0;
    vec3 pr = RotateZ(p0, iGlobalTime * rotationSpeed);
...
vec4 cubicTex(vec2 uv)
{
    ...
    vec2 fl = floor(uv);
    uv = smooth + fl;
    return texture2D(iChannel2, (uv + 0.5)/iChannelResolution[2].xy, -100.0);	// use constant here instead?
}

vec4 tex3d(vec3 pos, vec3 normal)
{
	// loook up texture, blended across xyz axis based on normal.
	vec4 texX = texture2D(iChannel2, pos.yz);
	vec4 texY = texture2D(iChannel2, pos.xz);
	vec4 texZ = texture2D(iChannel2, pos.xy);
...
{
	// loook up texture, blended across xyz axis based on normal.
	vec4 texX = texture2D(iChannel3, pos.yz);
	vec4 texY = texture2D(iChannel3, pos.xz);
	vec4 texZ = texture2D(iChannel3, pos.xy);
	vec4 tex = mix(texX, texZ, abs(normal.z));
	tex = mix(tex, texY, abs(normal.y));//.zxyw;
	return tex;
}
...
{
    float grayPanels = texture2D(iChannel1, uv.xy * 0.25).z;
    float grayPanels2 = texture2D(iChannel1, uv.xy * 0.333 + vec2(0.1, 0.3)).z;
    grayPanels += grayPanels2;
    float grayPanels3 = texture2D(iChannel1, uv.xy * 0.1777*vec2(1.0,5.0) + vec2(0.432, 0.765)).z;
...
void mainImage( out vec4 fragColor, in vec2 fragCoord )
{
    ...
    //if (t <= maxDepth)
	{
        vec3 posRot = RotateZ(pos, iGlobalTime * rotationSpeed);
        ...
        {
            normal += (texture2D(iChannel1, posRot.xy * 0.27).x - 0.4) * 0.0625;
        ...
        if (radial >= outerRadius - 0.01)
        {
            ...
            {
                float checker = texture2D(iChannel1, uvr).x;
                float checkerL = texture2D(iChannel0, uvr*0.0625).x;
            ...
            float stripes = texture2D(iChannel1, uvr * vec2(0.0625, 1.0)).x;
    ...
    else
    {
        ...
        if (hit >= 0.0)
        {
            ...
            finalColor = texture2D(iChannel2, vec2(normal.z, normal.y)).xyz;
