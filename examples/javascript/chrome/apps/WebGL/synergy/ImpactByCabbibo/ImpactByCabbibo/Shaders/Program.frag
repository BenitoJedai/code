...
{
  return length(p)- s;//+ .1 * sin( p.x * p.y * p.z * 10. + iGlobalTime );//* (1.+ .4 * triNoise3D( p * .1 ,.3 ) + .2* triNoise3D( p * .3 ,.3 )) ;
}

float sdPlanet( vec3 p, float s )
{

    return length(p)- s + .1 * triNoise3D( p * .5 , .01 )+ .04 * sin( p.x * p.y * p.z * 10. + iGlobalTime );;//+ .03 * noise( sin(p) * 10. + p ) + .03 * sin( p.x * p.y * p.z * 10. + iGlobalTime )+ .02 * sin( p.x + p.y + p.z * 2. + iGlobalTime );//* (1.+ .4 * triNoise3D( p * .1 ,.3 ) + .2* triNoise3D( p * .3 ,.3 )) 
...
vec2 map( vec3 pos ){  
    ...
    vec3 rot = vec3( 0. );//vec3( iGlobalTime * .05 + 1., iGlobalTime * .02 + 2. , iGlobalTime * .03  );
    // Rotating box
...
float posToFloat( vec3 p ){
    ...
    float f = triNoise3D( p * .2 + vec3( iGlobalTime * .01 , 0. , 0.), .1 );
    return f;
...
vec4 overlayFog( vec3 ro , vec3 rd , vec2 screenPos , float hit ){
    ...
    //float nSize = .000002 * hit;
   	//float n = (noise(vec3(2.0*screenPos, abs(sin(iGlobalTime * 10. ))*.1))*nSize) -.5* nSize;
...
void mainImage( out vec4 fragColor, in vec2 fragCoord ){
    ...
    if( timeInLoop < impactTime ){ 
       ...
       // col = vec3( fragCoord.x ,  , 1. );
        //col = texture2D( iChannel0 , sin(fragCoord.xy * 10. )).xyz;// vec3( 1. , 1. ,1. ); 
    }
