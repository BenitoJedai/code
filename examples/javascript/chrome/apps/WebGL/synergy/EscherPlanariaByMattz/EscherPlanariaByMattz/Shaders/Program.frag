...
vec2 worm(in vec3 p) {
	...
	const float f = 6.0;
	const float a = 0.03;
	p.y += a*sin(f*p.z+5.0*iGlobalTime);
...
vec4 wormline(in vec3 p0, in vec3 d, in float a, in float b, in vec3 pos) {
	...
	vec3 p = (pos - (p0 + (a + mod(4.0*iGlobalTime, b-a))*z))*mat3(x,y,z);
...
vec4 raymarch( in vec3 ro, in vec3 rd ) {
	...
	vec3 t = vec3(4.0, 0.0, 4.0) + sin(iGlobalTime)*vec3(4.0, 5.0, 0.0);
...
vec4 raytrace(in vec3 ro, in vec3 rd) {
	...
	if (h.w < farval) {
		...
		vec3 e = textureCube(iChannel0, R*reflect(rd, h.xyz)).zyx;
		col = mix(l, e, 0.18);
	}	
...
#if 1
	cam += vec3(0.0, 0.1*sin(1.3*iGlobalTime), 0.0);
	uv.x += (2.0 + 1.0*sin(0.04*uv.y+0.7*iGlobalTime+3.0)) * sin(0.05*uv.y + 2.0*iGlobalTime);
#endif
