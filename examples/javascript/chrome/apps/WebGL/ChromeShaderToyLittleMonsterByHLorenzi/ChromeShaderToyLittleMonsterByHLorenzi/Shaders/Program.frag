...
{
	vec2 bodyUV = uv *
		vec2(cos(iGlobalTime * 5.0) * 0.025 + 1.0,
			 1.0 - (cos(iGlobalTime * 5.0) * 0.025)) -
		vec2(0, (cos(iGlobalTime * 5.0) * 0.02));
	...
	vec2 armUV = uv - vec2((cos(iGlobalTime * 5.0) * 0.02), (cos(iGlobalTime * 5.0) * 0.02));
	vec2 arm2UV = uv - vec2(-(cos(iGlobalTime * 5.0) * 0.02), (cos(iGlobalTime * 5.0) * 0.02));
	
	float armPosTime = radians(100.0) + cos(iGlobalTime * 13.0) * radians(10.0);
	...
	if (sqrlen(eyeUV - vec2(0,0.2)) < 0.4 * 0.4) {
		...
		return vec4(1,1,0.7,1);
			  ...
			  bodyUV.y >
			   	getBezier(vec2(-0.3,-0.3), vec2(0,-0.6 + cos(iGlobalTime * 2.0) * 0.15), vec2(0.3,-0.3), (bodyUV.x + 0.3) / 0.6).y) {
		if (sqrlen(bodyUV - vec2(0,-0.5 + cos(iGlobalTime * 2.0) * 0.05)) < 0.2 * 0.2) {
