...
vec2 scene(vec3 p) {
	...
	scene_noise = pnoise_2d((p.xz * vec2(1.5, 1.) - vec2(mix(0., -mod(iGlobalTime, 30.) / 1.5, heaven), mix(0., mod(iGlobalTime, 30.), heaven + 1. - abs(sign(room - 2.))) * mix(1., .3, heaven)) + p.y) * mix(.75, 3. - room, abs(sign(room - 1.))));
	if(room == 0.) {
		...
		temp_17 = vec2(dot(p + vec3(0., p.z / 4., 0.), vec3(0., 1., 0.)) + 1. + clamp(scene_noise + sin(p.x + (mod(iGlobalTime, 30.) + 1.) * 2.) * .1, -.3, .6) * .4, 5.);
	}
...
vec3 camera_turn(vec3 p) {
	float v = max(mod(iGlobalTime, 30.) - 8., 0.) / 3.5;
...
void mainImage(out vec4 fragColor, in vec2 fragCoord) {
	...
	vec3 origin = camera_turn(vec3(0., .5, -4. - pow(clamp(1. - mod(iGlobalTime, 30.) / 10., 0., 1.), 3.) * 100. + sin(max(mod(iGlobalTime, 30.) - 10., 0.) / 4.) * .1) + vec3(eyeoff, 0., 0.));
	...
	if(cur.dist < .050) {
		fragColor = vec4(mix(mix(vec3(1., 1., 1.), shade(cur), clamp((mod(iGlobalTime, 30.) - .5) / 2., 0., 1.)), vec3(0.), smoothstep(0., 1., (radsq - .1) * 5.)), 1.);
	} else {
		vec3 cursky = sky(cur.origin, cur.dir);
		fragColor = vec4(mix(mix(vec3(1., 1., 1.), mix(cursky, apply_fog(cursky, cur, 20., 1.), min(cur.total_dist * 20., 1.)), clamp((mod(iGlobalTime, 30.) - .5) / 2., 0., 1.)), vec3(0.), smoothstep(0., 1., (radsq - .1) * 5.)), 1.);
	}
}
