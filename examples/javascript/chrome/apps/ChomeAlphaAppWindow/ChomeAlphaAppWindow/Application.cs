...
namespace ChomeAlphaAppWindow
{
	...
	{
		//......488: { SourceMethod = Void .ctor(ChomeAlphaAppWindow.HTML.Pages.IApp), i = [0x00f5] brtrue.s   +0 -1 }
		//414: erase { SourceMethod = Void .ctor(ChomeAlphaAppWindow.HTML.Pages.IApp), offset = -2, x = [0x00ef] ldsfld     +1 -0 }
		//414: erase { SourceMethod = Void .ctor(ChomeAlphaAppWindow.HTML.Pages.IApp), offset = -1, x = [0x00f4] dup        +2 -1 }
		//414: erase { SourceMethod = Void .ctor(ChomeAlphaAppWindow.HTML.Pages.IApp), offset = 0, x = [0x00f5] brtrue.s   +0 -1 }
		//414: erase { SourceMethod = Void .ctor(ChomeAlphaAppWindow.HTML.Pages.IApp), offset = 1, x = [0x00f7] pop        +0 -1 }
		//414: erase { SourceMethod = Void .ctor(ChomeAlphaAppWindow.HTML.Pages.IApp), offset = 5, x = [0x0108] dup        +2 -1 }
		//414: erase { SourceMethod = Void .ctor(ChomeAlphaAppWindow.HTML.Pages.IApp), offset = 6, x = [0x0109] stsfld     +0 -1 }
		//2024:02:01 RewriteToAssembly error: System.ArgumentException: Value does not fall within the expected range.
		//   at jsc.ILInstruction.ByOffset(Int32 i) in X:\jsc.internal.git\compiler\jsc\CodeModel\ILInstruction.cs:line 1177
			...
			if (self_chrome_socket != null)
			{
				...
				chrome.app.runtime.Launched +=
					async delegate
					{
						...
						new chrome.Notification { Message = "it works when 'Enable experimental canvas features'" };
						// showed
