...
namespace Plupload.PngEncoder
{
    ...
    public class DeflaterEngine : DeflaterConstants
    {
			...
			*/
            ...
            while (flag1)
            {
				...
				*/
                ...
                flag1 = ((curMatch = (prev[curMatch & WMASK] & 0xffff)) > limit && --chainLength != 0);
        ...
        bool DeflateStored(bool flush, bool finish)
        {
            ...
            var flag2 = (blockStart < WSIZE && storedLength >= MAX_DIST);
            var flag1 =
                (storedLength >= DeflaterConstants.MAX_BLOCK_SIZE) || // Block is full
                flag2 ||   // Block may move out of window
                flush;
        ...
        bool DeflateFast(bool flush, bool finish)
        {
            ...
            var flag0 = lookahead >= MIN_LOOKAHEAD;
            var flag1 = flag0 || flush;
            ...
            while (flag1)
            {
                ...
                var flag3a = lookahead >= MIN_MATCH;


                hashHead = InsertString();

                var flag3b = hashHead != 0;
        ...
        bool DeflateSlow(bool flush, bool finish)
        {
            ...
            while (lookahead >= MIN_LOOKAHEAD || flush)
            {
                ...
                if (lookahead >= MIN_MATCH)
                {
                    ...
                    var flag833a = strategy != DeflateStrategy.HuffmanOnly;
                    var flag833b = hashHead != 0;
                    var flag833c = strstart - hashHead <= MAX_DIST;

                    var flag833 = flag833a &&
                        flag833b &&
                        flag833c &&
                        FindLongestMatch(hashHead);
                    ...
                    if (flag833)
                    {
                        ...
                        var flag845a = matchLen <= 5;
                        var flag845c = strategy == DeflateStrategy.Filtered;
                        var flag845e = strstart - matchStart > TooFar;

                        var flag845d = (matchLen == MIN_MATCH && flag845e);

                        var flag845b = (flag845c || flag845d);
                        var flag845 = flag845a && flag845b;
                        if (flag845)
                        {
                            matchLen = MIN_MATCH - 1;
                        }
                ...
                var flag856 = (prevLen >= MIN_MATCH) && (matchLen <= prevLen);
                if (flag856)
                {
#if DebugDeflation
					if (DeflaterConstants.DEBUGGING) 
