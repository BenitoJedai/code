...
namespace WebGLBossHarvesterByOutsideOfSociety
{
    ...
    public sealed class Application
    {
        ...
        private static void InitializeContent()
        {
			...
			var cameraTarget = new THREE.Vector3(0,300,0);
			...
			function init() {
				...
				var floorMaterial = new THREE.MeshBasicMaterial( {wireframe: true, color: 0x333333} );
				floor = new THREE.Mesh(plane, floorMaterial);
				floor.rotation.x = -Math.PI/2;
				scene.add(floor);

				// model
				var loader = new THREE.JSONLoader();
				loader.load( "harvester.js", harvesterLoaded );
			...
			function harvesterLoaded (geometry) {
				...
				var material = new THREE.MeshBasicMaterial( { color: 0xffffff, wireframe: true, opacity: 0.25, transparent: true, skinning: true } );

				harvesterMesh = new THREE.SkinnedMesh( geometry, material );
				scene.add( harvesterMesh );

				THREE.AnimationHandler.add( geometry.animation );
				animation = new THREE.Animation( harvesterMesh, "walk1" );
				animation.play();
