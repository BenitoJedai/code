...
[<Sealed>]
 type Application(e : IHTMLElement) = do
    ...
    Native.Document.title <- "HelloFSharpTeam"
    ...
    let header_h2 = new IHTMLElement(IHTMLElement.HTMLElementEnum.h2)

    header_h2.innerText <- "Hello FSharp team!"
    header_h2.style.textDecoration <- "underline"

    header_h2.add_onclick(
        fun (e) ->
            Native.Window.alert("Howdy!")
    )

    header.Add(header_h2)
    c.Add(header)
