...
{
	public sealed class TheCloudEffect : Sprite
	{
		// HD :)
		...
		public const int STAGE_W = DefaultWidth;
		public const int STAGE_H = DefaultHeight;
		...
		private void Initialize()
		{
			...
			// some branding
			var logo = KnownEmbeddedResources.Default["assets/ScriptCoreLib/jsc.png"].ToBitmapAsset().AttachTo(this).MoveTo(STAGE_W - 128, STAGE_H - 128);

			var logo_hit = new Sprite();
			logo_hit.graphics.beginFill(0);
			logo_hit.graphics.drawRect(STAGE_W - 128, STAGE_H - 128, 96, 96);
			logo_hit.AttachTo(this);
			logo_hit.buttonMode = true;
			logo_hit.useHandCursor = true;
			logo_hit.alpha = 0;
			logo_hit.click +=
				delegate
				{
