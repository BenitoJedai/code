...
using TestPushManager;
using TestPushManager.Design;
using TestPushManager.HTML.Pages;
using System.Diagnostics;

namespace TestPushManager
{
    /// <summary>
        ...
        public Application(IApp page)
        {
            ...
            if (Native.window.navigator.serviceWorker.controller == null)
            {
                ...
                new { }.With(async delegate
                {
                    ...
                    var r = await Native.window.navigator.serviceWorker.register();
                    
                    // service register! {{ pushRegistrationManager = null, ElapsedMilliseconds = 9813 }}
                    new IHTMLPre { "service register! " + new { r.pushRegistrationManager, sw.ElapsedMilliseconds } }.AttachToDocument();
