				...
				foreach (var CurrentElement in ElementsWithSource)
				{
					...
					using (a.context.ToTransientTransaction())
					{
							...
							(m, il_a) =>
							{
								...
								il_a.BeforeInstructions =
									e =>
									{
										...
										if (e.SourceMethod == ((Func<string>)NamedImageInformation.GetImageAssetPath).Method)
										{
											e.il.Emit(OpCodes.Ldstr, Variations.FromAssetsSource);
											e.il.Emit(OpCodes.Ret);
											e.Complete();
											return;
										}
						...
						if (ElementType == typeof(IHTMLImage))
						{
                            ...
                            var ActionScriptImage = a.Module.DefineType(
                                DefaultNamespace + ".ActionScript.Images." + name,
                                TypeAttributes.Public,
                                typeof(global::ScriptCoreLib.ActionScript.flash.display.Bitmap)
                            );

                            ActionScriptImage.DefineAttribute(
                                new EmbedAttribute("/" + Variations.FromAssetsSource),
                                typeof(EmbedAttribute)
                            );

                            ActionScriptImage.CreateType();
