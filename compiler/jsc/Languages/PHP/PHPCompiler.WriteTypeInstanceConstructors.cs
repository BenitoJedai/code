...
namespace jsc.Script.PHP
{
    partial class PHPCompiler
    {
        private void WriteTypeInstanceConstructors(Type z)
        {
            ...
            var InvalidBaseType = IsMultipleConstructorsNotSupported(z);
        ...
        private bool IsMultipleConstructorsNotSupported(Type z)
        {
            ...
            if (z.BaseType != null)
                ...
                {
                    var BaseTypeResolved = ResolveImplementation(z.BaseType) ?? z.BaseType;

                    if (BaseTypeResolved.ToScriptAttributeOrDefault().IsNative)
                        return true;


                    if (BaseTypeResolved.ToScriptAttributeOrDefault().ImplementationType != null)
                        return IsMultipleConstructorsNotSupported(BaseTypeResolved.ToScriptAttributeOrDefault().ImplementationType);
                            ...
                            if (BaseConstructorCommands.Length == 1)
                            {
                                ...
                                if (BaseConstructorCallTargetConstructor != null)
                                {
                                    if (BaseConstructorCallTargetConstructor.DeclaringType == z.BaseType.BaseType)
                                        return IsMultipleConstructorsNotSupported(z.BaseType);
                                }
