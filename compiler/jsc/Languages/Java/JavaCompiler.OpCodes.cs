    ...
    unsafe partial class JavaCompiler
    {
        private void CreateInstructionHandlers()
        {
            ...
            CIW[OpCodes.Rethrow] =
                delegate(CodeEmitArgs e)
                {
                    if (e.i.Flow.OwnerBlock.Clause != null
                        && e.i.Flow.OwnerBlock.Clause.CatchType == typeof(object)
                        && e.i.Flow.OwnerBlock.First.OpCode == OpCodes.Pop)
                    {
                        WriteKeywordSpace(Keywords._throw);
                        WriteKeywordSpace(Keywords._new);
                        Write("RuntimeException");
                        Write("(");
                        Write("\"\"");
                        WriteSpace(",");
                        WriteExceptionVar();
