...
namespace jsc.Script
{
	...
	public abstract partial class CompilerBase
	{
		...
		{
			var t = ResolveImplementation(m.DeclaringType) ?? m.DeclaringType;


			if (t == typeof(Type) || t.ToScriptAttributeOrDefault().Implements == typeof(Type))
				if (m.IsStatic)
		...
		protected virtual bool WriteTypeOf(ILBlock.Prestatement p, ILInstruction i)
		{

			return false;
		}

		public virtual void WriteCustomMethodCall(Type context, MethodInfo TargetMethod, params object[] e)
		{
		...
		public void WriteMethodCall(ILBlock.Prestatement p, ILInstruction i, MethodBase m)
		{
			try
			{
				...
				{
					#region TypeOfByNameOverrideAttribute
					var TypeOfParameter = i.StackBeforeStrict[0].SingleStackInstruction.TargetType;

					if (i.OwnerMethod.GetCustomAttributes<TypeOfByNameOverrideAttribute>().Where(k => k.Target == null || k.Target == TypeOfParameter).Any())
					{
						// override
					...
					// implementing typeof here instead of opcodes is optional
					if (WriteTypeOf(p, i))
						return;
				}
		...
		public abstract Type ResolveImplementation(Type t);
