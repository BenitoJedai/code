...
namespace ScriptCoreLib.PHP.IO
{
    ...
    public class FileSystemInfo
    {
        ...
        {
            IArray<int, string> ax = new IArray<int, string>();


            if (Native.API.is_readable(dir))
            {
            
            object h = Native.API.opendir(dir);
            ...
            //Native.Log("reading files");

            while (p != null)
            {
                string npath = dir + p;

                bool bFile = !Native.API.is_dir(npath);

                if (bFile)
                {
                    if (Native.API.is_readable(npath))
                    {
                        //Native.Log(" file: " + npath);
                    }
                    else
                    {
                        //Native.Log(" ?: " + npath);
